...
type AuthContextValue = {
  ...
  updateProfile: (patch: Pick<UserProfile, "name" | "address">) => Promise<void>;
...
import { clearAdminSocketRoom, clearUserSocketRoom } from "@/lib/socket";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        // Ignore a startup response from an account that has since logged out or changed.
  ...
  const applySession = (
    token: string,
    rawUser: { id: string; phone: string; name?: string; address?: string; role: string },
  ) => {
    clearAdminSocketRoom();
    clearUserSocketRoom();
  ...
  const updateProfile = async (patch: Pick<UserProfile, "name" | "address">) => {
    const token = getAuthToken();
    const response = await fetch(API_URL + "/auth/profile", {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
      body: JSON.stringify({ name: patch.name, address: patch.address || "" }),
    });
    const data = await response.json();
    if (!response.ok || !data.success) throw new Error(data.message || "Unable to save details");
    if (getAuthToken() === token) setUser(data.user);
  };
  ...
  const value: AuthContextValue = {
    user, hydrated, orders, loginOpen, setLoginOpen,
    register, login, updateProfile, addOrder, logout,
  };
