...
export function CartProvider({
  ...
  const syncWithBackend = async (): Promise<boolean> => {
    try {
      ...
      if (!token) {
        console.error("Cannot sync cart: authentication token missing");
        return false;
      }

      // The local cart is the source of truth for checkout. Rebuild the
      // server cart from scratch every time so stale items/tier names from
      // an older deployment can never poison the order.
      const clearResponse = await fetch(`${API_URL}/cart`, {
        method: "DELETE",
      ...
      if (!clearResponse.ok || !clearData.success) {
        console.error("Unable to clear backend cart:", clearData);
        return false;
      }

      for (const line of lines) {
        const priceType = resolvePriceTypeForLine(line);
        ...
        if (!response.ok || !data.success) {
          console.error(
            `Unable to synchronize ${line.name} (${line.variant}) as ${priceType}:`,
            data,
          );
          return false;
        }
