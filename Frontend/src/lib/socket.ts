...
let userLifecycleBound = false;
function joinUserRoom() {
  if (socket?.connected && userToken) socket.emit("join:user", userToken);
}
export function ensureUserSocketRoom(token: string) {
  userToken = token;
  const current = getSocket();
  if (!userLifecycleBound) { current.on("connect", joinUserRoom); userLifecycleBound = true; }
  joinUserRoom();
}
export function clearUserSocketRoom() {
  userToken = null;
  if (socket?.connected) socket.emit("leave:user");
  // Discard recoverable memberships and any queued authenticated events.
