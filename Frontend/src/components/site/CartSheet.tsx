...
export function CartSheet() {
  const { isOpen, setOpen, linesFor, setQty, remove, subtotalFor, countFor } = useCart();
  // Sip n Scoop is paused. The customer cart remains restaurant-only so unfinished
  // Sip n Scoop state can never interfere with normal food checkout.
  const activeSection = "restaurant" as const;
  const lines = linesFor(activeSection);
  const subtotal = subtotalFor(activeSection);
  const count = countFor(activeSection);
  const estimatedDelivery = lines.length ? restaurant.deliveryFee : 0;
          ...
          <>
            ...
            <div className="space-y-3 border-t bg-card p-5">
              ...
              <p className="text-[11px] text-muted-foreground">
                Final restaurant delivery fee is calculated from your location at checkout.
              </p>
