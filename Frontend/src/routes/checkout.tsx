    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          // Nominatim always returns a full readable `display_name`
          // even when the structured address fields above are sparse
          // (common in smaller towns/rural areas) — fall back to that
          // rather than leaving the address blank.
          if (line) {
            setAddress(line);
          } else if (data?.display_name) {
            setAddress(String(data.display_name).split(",").slice(0, 3).join(", "));
          }
        ...
        <aside className="lg:sticky lg:top-32 lg:h-fit">
          ...
          <div className="rounded-3xl border bg-card p-6 shadow-card">
            ...
            <div className="mt-4 flex items-center gap-2.5 rounded-2xl border border-primary/30 bg-accent px-4 py-3 text-sm">
              ...
              <span>
                <span className="font-bold">Minimum prep time: 25 mins</span>
                <span className="block text-xs text-muted-foreground">
                  Your food is made fresh to order — please allow at least 25
