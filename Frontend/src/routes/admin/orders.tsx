...
import { ensureAdminSocketRoom, getSocket } from "@/lib/socket";
...
function AdminOrders() {
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "restaurant" | "sip-n-scoop">("all");
  ...
  useEffect(() => {
    ...
    if (adminToken) ensureAdminSocketRoom(adminToken);
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      ...
      const inferredType = order.orderType || (order.items.some((item) => item.section === "sip-n-scoop") ? "sip-n-scoop" : "restaurant");
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {([
            ...
            ["sip-n-scoop", "🍨 Sip n Scoop"],
          ] as const).map(([value, label]) => (
            <button key={value} type="button" onClick={() => setTypeFilter(value)} className={`rounded-full border px-4 py-2 text-sm font-bold ${typeFilter === value ? "bg-primary text-primary-foreground" : "bg-background"}`}>{label}</button>
          ))}
        </div>
...
function OrderCard({
  ...
  const isSipNScoopOrder = order.orderType === "sip-n-scoop" || order.items.some(
    ...
    <div className="overflow-hidden rounded-2xl border bg-background shadow-sm">
      ...
      <div className="flex flex-col gap-4 border-b p-5 md:flex-row md:items-center md:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            {isSipNScoopOrder && (
              <span className="rounded-lg bg-violet-600 px-2.5 py-1 text-xs font-black tracking-wide text-white">
                🍨 SIP N SCOOP • PACKING / QUICK DELIVERY
              </span>
            )}
          </div>
      ...
      <div className="grid gap-6 border-b p-5 md:grid-cols-2">
        ...
        <div>
          ...
          {isSipNScoopOrder && order.deliveryDistanceKm != null && (
            <p className="mt-2 text-xs font-bold text-violet-700">📍 {order.deliveryDistanceKm} km • ₹10 quick delivery</p>
          )}
        </div>
      </div>
        ...
        <div className="space-y-2 text-sm">
          ...
          {(order.discountAmount ?? 0) > 0 && (
            <div className="flex justify-between font-bold text-green-700">
              <span>{order.couponCode || "Coupon"}</span>
      ...
      <div className="flex flex-col gap-3 p-5 md:flex-row md:items-center md:justify-between">
        ...
        >
          {STATUS_OPTIONS.map((status) => (
            <option key={status} value={status}>
              {isSipNScoopOrder && status === "confirmed" ? "Accepted" : isSipNScoopOrder && status === "preparing" ? "Packing" : formatStatus(status)}
            </option>
          ))}
        </select>
      </div>
