...
function AdminProducts() {
  ...
  const [sectionFilter, setSectionFilter] = useState<"all" | "restaurant" | "sip-n-scoop">("all");
  ...
  const openEditForm = (product: Product) => {
    ...
    setForm({
      ...
      section: product.section || "restaurant",
      image: product.image || "",
      regular:
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      if (sectionFilter !== "all" && (product.section || "restaurant") !== sectionFilter) return false;
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {([
            ...
            <button key={value} type="button" onClick={() => setSectionFilter(value)} className={`rounded-full border px-4 py-2 text-sm font-bold ${sectionFilter === value ? "bg-primary text-primary-foreground" : "bg-background"}`}>{label}</button>
          ))}
        </div>
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            {product.section === "sip-n-scoop" && (
              <span className="mt-2 inline-flex rounded-full bg-violet-100 px-2.5 py-1 text-[11px] font-bold text-violet-700">
                ⚡ Sip n Scoop
              </span>
            )}
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-background shadow-2xl">
        ...
        <div className="space-y-5 p-5">
          ...
          <FormField label="Store Section" required>
            <select
              value={form.section}
              onChange={(event) =>
                updateForm(
                  "section",
                  event.target.value as ProductForm["section"],
                )
              }
              className="form-input"
            >
              <option value="restaurant">Restaurant Menu</option>
              <option value="sip-n-scoop">Sip n Scoop — Quick Delivery</option>
            </select>
            <p className="mt-1.5 text-xs text-muted-foreground">
              Sip n Scoop items use the special ₹10 / 3 km delivery rule.
            </p>
          </FormField>
          ...
          <div>
            ...
            <p className="mb-3 text-xs text-muted-foreground">
              Use "Regular" for a single-price item. For portioned dishes,
              use Half + Full only. Quarter and Family are no longer supported.
            </p>
