...
app.use(helmet());
// Render has one trusted reverse-proxy hop; local development stays direct.
...
app.use((error, req, res, next) => {
  ...
  const status = error.name === "ValidationError" || error.name === "CastError" || error.type === "entity.parse.failed"
    ? 400 : error.status || 500;
  return res.status(status).json({ success: false, message: status < 500 ? "Invalid request" : "Unable to complete request" });
});
