...
export const initSocket = (httpServer, allowedOrigins) => {
  io = new Server(httpServer, {
    // Recover rooms and missed packets after short network/background-tab
    // interruptions. Durable REST reconciliation below still covers server
    // restarts where in-memory recovery state cannot survive.
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join:admin", async (token, acknowledge) => {
      try {
        if (!token) throw new Error("Missing admin token");

        const { decoded } = await verifySession(token, true);
    ...
    const leaveUserRooms = () => {
      for (const room of socket.rooms) if (room.startsWith("user-")) socket.leave(room);
    };
    socket.on("leave:user", leaveUserRooms);
    socket.on("join:user", async (token) => {
      leaveUserRooms();
      try {
        const { account } = await verifySession(token);
        if (socket.connected) socket.join("user-" + account._id);
      } catch { leaveUserRooms(); }
    });
  });
