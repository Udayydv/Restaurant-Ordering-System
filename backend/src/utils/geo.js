...
export const SIP_N_SCOOP_MAX_DISTANCE_KM = 3;
export const SIP_N_SCOOP_DELIVERY_FEE = 10;

export const getSipNScoopDeliveryQuote = (latitude, longitude) => {
  const lat = Number(latitude);
  const lng = Number(longitude);
  ...
  const km = distanceFromRestaurantKm(lat, lng);
  const deliverable = km <= SIP_N_SCOOP_MAX_DISTANCE_KM;

  return {
    distanceKm: km,
    deliveryFee: deliverable ? SIP_N_SCOOP_DELIVERY_FEE : null,
    deliverable,
  };
};
