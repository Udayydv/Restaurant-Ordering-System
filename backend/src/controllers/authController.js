...
export const updateProfile = async (req, res) => {
  ...
  if (typeof name !== "string" || name.trim().length > 100 || typeof address !== "string" || address.trim().length > 1000) {
    return res.status(400).json({ success: false, message: "Provide a name (up to 100 characters) and address (up to 1000 characters)" });
  }
  req.user.name = name.trim();
  req.user.defaultAddress = address.trim();
  await req.user.save();
