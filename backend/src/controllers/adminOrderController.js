...
// GET DURABLE NEW-ORDER ALERTS
// Uses Mongo ObjectIds as a durable cursor. Unlike Socket.io, this survives
// temporary disconnects, background-tab throttling and frontend remounts.
  ...
  try {
    ...
    if (!after) {
      ...
      return res.status(200).json({
        success: true,
        orders: [],
        cursor: latest ? String(latest._id) : "000000000000000000000000",
        latestCreatedAt: latest?.createdAt || null,
        hasMore: false,
      });
    }

    if (!/^[a-fA-F0-9]{24}$/.test(after)) {
      return res.status(400).json({
        success: false,
        message: "Invalid order alert cursor",
      });
    }

    const batchSize = 100;
    const afterId = new mongoose.Types.ObjectId(after);
    const rows = await Order.find({ _id: { $gt: afterId } })
      .populate("user", "name phone")
      .sort({ _id: 1 })
      .limit(batchSize + 1);

    const hasMore = rows.length > batchSize;
    const orders = hasMore ? rows.slice(0, batchSize) : rows;
    const cursor = orders.length
      ? String(orders[orders.length - 1]._id)
      : after;

    let latestCreatedAt = orders.length
      ? orders[orders.length - 1].createdAt
      : null;

    if (!latestCreatedAt) {
      const latest = await Order.findOne().sort({ _id: -1 }).select("createdAt");
      latestCreatedAt = latest?.createdAt || null;
    }
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to fetch order alerts",
    });
  }
};
...
export const getOrderHistoryByDate = async (req, res) => {
  try {
    ...
    const parsedDate = new Date(String(date) + "T00:00:00.000Z");
